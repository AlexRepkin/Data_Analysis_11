...
вычисление значений в двух функциях в отдельных процессах.
...
def main():
    # Создание менеджера контекста, гарантирующего, что ресурсы будут освобождены по завершении блока with
    with Manager() as manager:
        # Создание разделяемого словаря, используемого для обмена данными между процессами.
        ...
        # Создание процессов для вычисления сумм.
        first_thread = Process(target=second_var, args=(results,))
        second_thread = Process(target=third_var, args=(results,))
